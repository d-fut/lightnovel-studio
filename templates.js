// ===== Prompt Templates =====

const DEFAULT_TEMPLATES = {
  concept: {
    name: '01 コンセプト・世界観',
    system: `あなたはSFコンセプトデザイナーです。日本のラノベ市場に精通し、独創的かつ市場性のある世界観を設計します。`,
    user: `以下の条件でラノベの世界観コンセプトを正確に10案生成せよ。

## 基本情報
- 総字数目標：{total_chars}字
- ジャンル：{genre}
- テーマキーワード：{theme}
- 全体感情曲線：{arc_type}

## よく使われる設定例（参考・差別化に使え）
異世界転生、チート能力、魔法学校、ハーレム、ギルド、勇者・魔王、VRゲーム世界、タイムループ、記憶喪失、契約精霊

## 禁止：上記の設定をそのまま使うこと

## 出力フォーマット（Markdownのみ・JSONは使わない）
10案を以下の形式で繰り返せ。前置き・説明・コードブロック不要。

---
### 案N ｜ スコア：X/10

**キャッチコピー：**（30字以内）
**世界の核心ルール：**（3行以内）
**主人公が直面する問い：**（1文）
**差別化ポイント：**（1文）
**感情曲線との適合：**（選択された曲線タイプと物語の符合を1文で）
**スコア根拠：**（1文）

---
（案ごとに繰り返す）

## 制約
- 設定のための設定にしない
- 「問い」は読者が自分事にできるものに限定
- スコアは客観的に評価し、8点以上は最大2案まで
- Markdownのみ返せ。JSONは使わない`
  },

  plot: {
    name: '02 プロット構築',
    system: `あなたはプロ脚本家です。三幕構成でプロットを設計します。感情的煽りより論理的必然性を重視します。`,
    user: `以下のコンセプトから三幕構成プロットを設計せよ。

## コンセプト決定稿
{concept}

## 基本情報
- 総字数目標：{total_chars}字
- ジャンル：{genre}
- テーマ：{theme}
- 全体感情曲線：{arc_type}
- 固有名詞リスト：{nouns}

## 各幕の字数配分（合計が総字数と一致すること）
- 第一幕（序）：{act1_chars}字（総字数の{act1_pct}%）
- 第二幕（破）：{act2_chars}字（総字数の{act2_pct}%）
- 第三幕（急）：{act3_chars}字（総字数の{act3_pct}%）
- クライマックス位置：{climax_pos}字地点（第二幕終盤〜第三幕冒頭）

## 出力フォーマット（Markdownのみ・JSONは使わない）
各幕を以下の見出し構造で出力せよ。前置き・説明・コードブロック不要。

### 第一幕（序）｜{act1_chars}字
**日常と欠如：**（ここに記述）
**事件（引き金）：**（ここに記述）
**伏線：**（最低1本、ここに記述）

### 第二幕（破）｜{act2_chars}字
**主人公の選択と代償：**（ここに記述）
**中間点（価値観の転換）：**（ここに記述）
**最大の危機：**（ここに記述）

### 第三幕（急）｜{act3_chars}字
**クライマックス（{climax_pos}字地点・論理的決断として）：**（ここに記述）
**結末と余韻：**（ここに記述）

## 制約
- 感動を「泣ける」で表現しない
- クライマックスは総字数の70〜80%地点
- 伏線は第一幕で必ず1本以上
- Markdownのみ返せ。JSONは使わない`
  },

  characters: {
    name: '03 キャラクター設計',
    system: `あなたはキャラクター設計の専門家です。アンサンブルの一貫性とテーマとの連動を重視します。`,
    user: `以下の情報から登場人物を設計せよ。

## プロット
{plot}

## テーマ
{theme}

## 固有名詞リスト（既存）
{nouns}

## 出力フォーマット（Markdownのみ・JSONは使わない）
各キャラクターを以下の見出し構造で出力せよ。前置き不要。

---
## キャラクター名（年齢・役職）

**表の動機：**（他者への説明）
**裏の動機：**（本人も認識が薄い）
**口癖・言動の癖：**（具体的に）
**主人公との対比：**（対比構造）
**絶対に言わないセリフ：**
- （例1）
- （例2）

---
（キャラクターごとに繰り返す）

最後に以下を付記：
## アンサンブル評価
（全キャラがテーマへの答えのバリエーションになっているか評価）

## 制約
- 「論理的な外面 × 感情的な内核」の構造を全キャラに持たせる
- 主人公・ライバル・協力者・鍵キャラの4名以上
- Markdownのみ返せ。JSONは使わない`
  },

  chapters: {
    name: '04 節立て・構成',
    system: `あなたは構成編集者です。読者を飽きさせない節構成を設計します。`,
    user: `以下のプロットから節構成表を作成せよ。

## プロット
{plot}

## 字数配分（厳守）
- 総字数：{total_chars}字
- 第一幕分：{act1_chars}字 → この幕を担当する節の合計がこの字数になること
- 第二幕分：{act2_chars}字 → この幕を担当する節の合計がこの字数になること
- 第三幕分：{act3_chars}字 → この幕を担当する節の合計がこの字数になること
- **全節の目安字数の合計が{total_chars}字になること（必須）**

## 節数の目安
- 1節あたり2,000〜3,000字
- 第一幕：{act1_chap_count}節程度
- 第二幕：{act2_chap_count}節程度
- 第三幕：{act3_chap_count}節程度

## 固有名詞リスト
{nouns}

## 感情表記ルール
各節の感情方向を以下で表記すること：
- ↑：感情が上昇する節（希望・高揚・成功）
- →：感情が横ばいの節（日常・情報提供・移動）
- ↓：感情が下降する節（挫折・喪失・緊張）

**制約：全節の並びで↑↑や↓↓など同じ感情記号が連続しないこと（必ず変化を持たせる）**

## 出力フォーマット（Markdownのみ・JSONは使わない）
節ごとに以下の構造で出力せよ。幕の区切りに見出し3を入れること。

### 第一幕（序）
---
#### 第N節「タイトル」｜目安X字 ｜ 感情：↑

**視点人物：**（名前）
**主な出来事：**（内容）
**冒頭の引き：**（内容）
**末尾フック：**（内容）【疑問 / 誤解 / 選択】

---
（節ごとに繰り返す）

### 第二幕（破）
（同様）

### 第三幕（急）
（同様）

---
## 字数合計確認
全節の目安字数の合計：X字（総字数{total_chars}字との差：Y字）

## 制約
- 各節は単独で読んでも「問い」が残る構造
- 連続する2節で同じ感情トーンを使わない
- 感情記号↑↓→が隣り合う節で重複しないこと
- 節末フックは必ず「疑問」「誤解」「選択」のいずれかに分類して【】内に記載
- 字数合計確認を必ず末尾に付記すること
- Markdownのみ返せ。JSONは使わない`
  },

  draft: {
    name: '05 章下書き',
    system: `あなたは日本のラノベ作家です。読者を引き込む文章を書きます。感情の直接描写を避け、描写と行動で感情を表現します。`,
    user: `以下の仕様でこの章の本文を生成せよ。

## 章仕様
{chapter_spec}

## コンセプト・テーマ
{theme}

## 固有名詞リスト
{nouns}

## これまでの章要約
{summaries}

{style_section}

## 禁止表現リスト
- 「胸が痛んだ」「心が震えた」「感情が高ぶった」等の感情直接描写
- 「〜のだった」調の冗長な締め
- 比喩の連続使用（1段落1個まで）
- キャラクターの独白での自己説明
- 「まるで〜のような〜のような」の入れ子比喩

## 出力
{target_chars}字程度。会話・描写・内省のバランスを均等に。
本文のみ出力し、末尾に自己チェックを付記：
[ ] 禁止表現なし
[ ] 末尾フック達成：{ending_hook}
[ ] 固有名詞の表記一致`
  },

  consistency: {
    name: '06 整合性チェック',
    system: `あなたは厳格な編集者です。感情的評価を一切せず、論理的矛盾のみを報告します。`,
    user: `以下の原稿を整合性チェックせよ。

## 固有名詞リスト
{nouns}

## キャラクター設計
{characters}

## 原稿（要約版）
{manuscript}

## チェック項目
1. 時系列矛盾（日時・年齢・経過時間）
2. キャラクター行動の動機一貫性
3. 世界観ルールの逸脱
4. 伏線の回収漏れ
5. 固有名詞の表記ゆれ

## 出力フォーマット（Markdownのみ・JSONは使わない）
以下の構造で出力せよ。問題なしの項目も必ず記載。

## 整合性チェック結果

### 時系列矛盾
（問題ありの場合）**箇所：**… **内容：**… **修正案：**…
（問題なしの場合）問題なし

### キャラクター動機の一貫性
（同上）

### 世界観ルールの逸脱
（同上）

### 伏線の回収漏れ
（同上）

### 固有名詞の表記ゆれ
（同上）

感想・評価・褒め言葉は一切不要。Markdownのみ返せ。JSONは使わない。`
  },

  revision: {
    name: '07 文体統一・改稿',
    system: `あなたは文体統一の専門家です。指定されたスタイルガイドに厳密に従います。`,
    user: `以下の本文をスタイルガイドに従いリライトせよ。

## スタイルガイド
- 文末バリエーション：「〜た」「〜る」「〜だ」を3:2:1の比率
- 一文の最大字数：60字
- 会話文の直後に必ず1行の非言語描写
- 同一語句の1段落内重複禁止

## トーン
{tone}

## 原稿
{manuscript}

リライト後の本文のみ出力。
末尾に変更サマリーを3行以内で付記。`
  },

  polish: {
    name: '08 最終仕上げ',
    system: `あなたは校正専門家です。誤字脱字・文法・一貫性を厳密にチェックします。`,
    user: `以下の本文を校正せよ。

## 固有名詞リスト（表記の正解）
{nouns}

## 原稿
{manuscript}

## チェック（優先順位順）
1. 誤字脱字・送り仮名
2. 読点の過不足（一文に3個以上は分割）
3. 同一語句の3行以内重複
4. セリフの語尾キャラクター一貫性

冒頭に「修正N件」と記載後、修正済み本文を出力。`
  },

  visual_character: {
    name: 'VIS キャラビジュアル',
    system: `あなたは画像生成AIのプロンプトエンジニアです。`,
    user: `以下のキャラクター設定から画像生成プロンプトを作成せよ。

## キャラクター情報
{character}

## 画風スタイル
{style}

## 出力フォーマット（Markdownのみ・JSONは使わない）

## キャラクタービジュアルプロンプト

### Stable Diffusion
\`\`\`
（SD形式プロンプト）
\`\`\`

### NovelAI
\`\`\`
（NAI形式プロンプト）
\`\`\`

### Midjourney
\`\`\`
/imagine prompt: （MJ形式プロンプト）
\`\`\`

### ネガティブプロンプト（共通）
\`\`\`
（ネガティブプロンプト）
\`\`\`

Markdownのみ返せ。JSONは使わない。`
  },

  visual_scene: {
    name: 'VIS シーン挿絵',
    system: `あなたは画像生成AIのプロンプトエンジニアです。`,
    user: `以下の情報から最も盛り上がるシーンの挿絵プロンプトを作成せよ。

## プロット・クライマックス
{climax}

## 登場キャラクター
{characters}

## 画風スタイル
{style}

## 出力フォーマット（Markdownのみ・JSONは使わない）

## シーン挿絵プロンプト

**シーン説明：**（日本語で簡潔に）

### Stable Diffusion
\`\`\`
（SD形式プロンプト）
\`\`\`

### NovelAI
\`\`\`
（NAI形式プロンプト）
\`\`\`

### Midjourney
\`\`\`
/imagine prompt: （MJ形式プロンプト）
\`\`\`

### ネガティブプロンプト（共通）
\`\`\`
（ネガティブプロンプト）
\`\`\`

Markdownのみ返せ。JSONは使わない。`
  },

  summary: {
    name: '章要約（自動）',
    system: `あなたは編集者です。節の要約を正確に作成します。`,
    user: `以下の節本文を3〜5行で要約せよ。
キャラクターの行動・判断・変化のみを記述。感想・評価不要。

## 本文
{draft}

要約のみ出力（前置き不要）。`
  },

  episode_title: {
    name: '話サブタイトル生成',
    system: `あなたはラノベの編集者です。話のサブタイトルを簡潔につけます。`,
    user: `以下の本文の内容を表す20字以内のサブタイトルを1つだけ生成せよ。
鍵括弧なし、前置き不要、タイトルのみ出力。

## 本文（冒頭抜粋）
{manuscript}`
  }
};

// Custom templates (stored in IndexedDB)
const TemplateManager = {
  _templates: null,

  async load() {
    if (this._templates) return this._templates;
    const saved = await dbGet('custom_templates');
    this._templates = saved || {};
    return this._templates;
  },

  async get(key) {
    const saved = await this.load();
    return saved[key] || DEFAULT_TEMPLATES[key] || null;
  },

  async set(key, template) {
    const saved = await this.load();
    saved[key] = template;
    await dbSet('custom_templates', saved);
  },

  async reset(key) {
    const saved = await this.load();
    delete saved[key];
    await dbSet('custom_templates', saved);
  },

  getDefault(key) {
    return DEFAULT_TEMPLATES[key] || null;
  },

  getAllKeys() {
    return Object.keys(DEFAULT_TEMPLATES);
  }
};

// Build prompt with variable substitution
function buildPrompt(template, vars) {
  let text = template;
  for (const [k, v] of Object.entries(vars)) {
    text = text.replaceAll(`{${k}}`, v || '（未設定）');
  }
  return text;
}
